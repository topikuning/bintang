...
def test_budget_status_with_marketing_excludes():
    ...
    bs = budget_status(p, total_out=Decimal("900"), marketing_actual=Decimal("200"))
...
def test_budget_status_overbudget_only_if_non_marketing_exceeds():
    ...
    expected_pct = (Decimal(expected_non_mkt) / Decimal("6762681560") * 100).quantize(
        Decimal("0.01")
    )
    ...
    db.add_all(
        [
            ...
            Transaction(
                ...
                created_by_id=u.id,  # DRAFT, di-include di ACTIVE
            ),
        ]
    )
    await db.commit()
