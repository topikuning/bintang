...
async def _seed_2_projects(db, *, role=UserRole.PROJECT_ADMIN):
    ...
    await db.flush()
    u = User(
        email="u@x", name="U", password_hash=hash_password("x"), role=role, scope_all_projects=True
    )
    db.add(u)
    await db.flush()
    return co, p1, p2, u
...
async def test_po_non_draft_project_change_blocked_for_non_super(db):
    ...
    with pytest.raises(HTTPException) as exc:
        await update_po(pid=po.id, payload=POUpdate(project_id=p2.id), db=db, user=u)
...
async def test_po_superadmin_godmode_edit_non_draft(db):
    ...
    db.add(po)
    await db.commit()
    out = await update_po(pid=po.id, payload=POUpdate(project_id=p2.id), db=db, user=u)
    assert out.project_id == p2.id
...
async def test_invoice_draft_edit_project_works(db):
    ...
    out = await update_invoice(
        iid=inv.id,
        payload=InvoiceUpdate(project_id=p2.id),
        db=db,
        user=u,
    )
    assert out.project_id == p2.id
...
async def test_tx_superadmin_godmode_edit_verified(db):
    ...
    out = await update_transaction(
        tid=tx.id,
        payload=TransactionUpdate(project_id=p2.id),
        db=db,
        user=u,
    )
    assert out.project_id == p2.id
