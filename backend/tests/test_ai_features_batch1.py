...
async def _seed(db):
    ...
    db.add(p)
    await db.flush()
    u = User(email="u@x", name="U", password_hash=hash_password("x"), role=UserRole.PROJECT_ADMIN)
    db.add(u)
    await db.flush()
    return co, p, u
...
async def test_category_suggest_returns_valid_id(db, monkeypatch):
    ...
    async def _fake_claude(**kw):
        return (
            "",
            {"category_id": cat_id, "confidence": 0.92, "reason": "deskripsi sebut material"},
            50,
            20,
        )

    monkeypatch.setattr(llm, "_call_claude", _fake_claude)
    monkeypatch.setattr(llm, "_resolve_model", lambda h: ("claude-haiku-4-5", "claude"))
...
async def test_category_suggest_rejects_hallucinated_id(db, monkeypatch):
    ...
    async def _fake_claude(**kw):
        return ("", {"category_id": 999999, "confidence": 0.9, "reason": "halusinasi"}, 10, 10)
...
async def test_po_cover_generates_text(db, monkeypatch):
    ...
    async def _fake_claude(**kw):
        return ("Kepada PT Vendor X,\n\nSurat pengantar PO-001...", None, 200, 150)
...
async def test_justify_by_cash_request_id(db, monkeypatch):
    ...
    async def _fake_claude(**kw):
        return ("Pengajuan untuk pembelian paku tahap finishing minggu ini.", None, 80, 40)
