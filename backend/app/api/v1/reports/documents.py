...
async def report_invoices(
    ...
    status_disp = (
        status.value
        if status
        else ("Semua aktif (excl. DRAFT/CANCELLED)" if not include_drafts else "Semua")
    )
    ...
    if type is None:
        ...
        scope_line = (
            f"Periode {period_label} · {proj_label} · Pisah Hutang vs Piutang · {status_disp}"
        )
    else:
        ...
        summary = [
            {
                "label": f"Total Nilai {arah}",
                "value": f"Rp {_fmt_idr(total)}",
...
async def report_debts(
    ...
    as_of: date_type | None = Query(
        None,
        description="As-of date untuk aging. Default: hari ini.",
    ),
    db: AsyncSession = Depends(get_db),
    user: User = Depends(get_current_user),
) -> Response:
    ...
    if inv_ids:
        ...
        paid_map_debts = {iid: Decimal(amt or 0) for iid, amt in (await db.execute(bulk_q)).all()}
    for inv in invs:
        ...
        umur_disp = (
            ...
            else (str(days_overdue) if days_overdue >= 0 else f"({-days_overdue})")
        )
        ...
        rows.append(
            [
                inv.number,
                "Hutang" if inv.type == InvoiceType.IN else "Piutang",
                _fmt_date(inv.due_date),
                umur_disp,
                proj_map.get(inv.project_id).name if proj_map.get(inv.project_id) else "-",
                inv.party_name or "-",
    ...
    summary = [
        {
            "label": "Sisa Hutang",
            "value": f"Rp {_fmt_idr(sum_remaining_in)}",
            "sub": f"dari Rp {_fmt_idr(sum_total_in)} ({n_in} invoice)",
        },
        {
            "label": "Sisa Piutang",
            "value": f"Rp {_fmt_idr(sum_remaining_out)}",
            "sub": f"dari Rp {_fmt_idr(sum_total_out)} ({n_out} invoice)",
        },
        {
            "label": "Net Position",
            "value": f"Rp {_fmt_idr(sum_remaining_out - sum_remaining_in)}",
            "sub": "Piutang − Hutang",
        },
        {"label": "Aging Hutang", "value": "lihat detail", "sub": _aging_str(aging_in)},
        {"label": "Aging Piutang", "value": "lihat detail", "sub": _aging_str(aging_out)},
        {
            "label": "Total Invoice Aktif",
            "value": str(len(invs)),
    ...
    scope_line = (
        f"{proj_label} · As-of {_fmt_date(today)} · Status aktif (Issued, Partial, Overdue)"
    )
    ...
    return await _output(
        format,
        title="Laporan Hutang & Piutang (Aging)",
        headers=headers,
        rows=rows,
        cols=cols,
        filters={
            "Proyek": proj_label,
            "As-of": _fmt_date(today),
        ...
        summary=summary,
        scope_line=scope_line,
        detail_label="Daftar Invoice Aktif (urut jatuh tempo)",
        footer_row=footer_row,
...
async def report_pos(
    ...
    summary = [
        {"label": "Total Nilai PO", "value": f"Rp {_fmt_idr(total)}", "sub": f"{len(pos)} dokumen"},
        {
            "label": "Disetujui",
            "value": str(n_approved),
