    ...
    summary = [
        {
            ...
            "value": f"Rp {_fmt_idr(saldo_awal)}",
        ...
        {"label": "Total Pemasukan", "value": f"Rp {_fmt_idr(sum_in)}", "sub": f"{n_in} transaksi"},
        {
            ...
            "value": f"Rp {_fmt_idr(sum_out)}",
            "sub": f"{n_out} transaksi",
        },
        {
            "label": "Perubahan (Surplus/Defisit)",
            "value": f"Rp {_fmt_idr(saldo_perubahan)}",
            "sub": "Surplus" if saldo_perubahan >= 0 else "Defisit",
        },
        {
            "label": "Saldo Akhir Periode",
            "value": f"Rp {_fmt_idr(saldo_akhir)}",
...
async def report_transactions(
    ...
    if cat_ids:
        cat_map = {
            c.id: c
            for c in (await db.execute(select(Category).where(Category.id.in_(cat_ids))))
            .scalars()
            .all()
        }
    ...
    if type is None:
        ...
        summary = [
            ...
            {
                "label": "Selisih (IN−OUT)",
                "value": f"Rp {_fmt_idr(saldo)}",
        ...
        footer_row = ["TOTAL", "", "", "", "", "", _fmt_idr(sum_in), _fmt_idr(sum_out)]
        title = "Laporan Transaksi (Gabungan)"
        scope_line = f"Periode {period_label} · {proj_label} · Gabungan IN+OUT · {status_disp}"
    else:
        ...
        scope_line = f"Periode {period_label} · {proj_label} · {arah_label} · {status_disp}"
...
async def report_direct_expenses(
    ...
    for t in txs:
        ...
        if items:
            for it in items:
                ...
                n_items += 1
                cat_name = (
                    cat_map.get(it.category_id).name
                    if it.category_id and cat_map.get(it.category_id)
                    else "-"
                )
            ...
            total += amt
            cat_name = (
                cat_map.get(t.category_id).name
                if t.category_id and cat_map.get(t.category_id)
                else "-"
            )
            cat_value[cat_name] = cat_value.get(cat_name, _D("0")) + amt
            rows.append(
    ...
    summary = [
        {
            ...
            "value": f"Rp {_fmt_idr(total)}",
            "sub": f"{len(txs)} tx · status VERIFIED"
            if not include_drafts
            else f"{len(txs)} tx (include draft)",
        },
        {"label": "Jumlah Item", "value": str(n_items), "sub": "rincian per kategori"},
        {
    ...
    cat_label = (
        cat_map.get(category_id).name
        if category_id and cat_map.get(category_id)
        else "Semua kategori"
    )
    period_label = (
    ...
    return await _output(
        ...
        summary=summary,
        scope_line=scope_line,
        detail_label="Detail Beban Langsung",
        footer_row=footer_row,
