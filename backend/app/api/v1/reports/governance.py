...
async def report_budget(
    ...
    if co_ids:
        company_map = {
            c.id: c
            for c in (await db.execute(select(Company).where(Company.id.in_(co_ids))))
            .scalars()
            .all()
        }
    ...
    if proj_ids_list:
        ...
        spent_map = {pid: Decimal(amt or 0) for pid, amt in (await db.execute(spent_q)).all()}
        ...
        marketing_map: dict[int, Decimal] = {
            ...
            for pid, amt in (await db.execute(_flag_q(Category.is_marketing))).all()
        }
        profit_share_map: dict[int, Decimal] = {
            pid: Decimal(amt or 0)
            for pid, amt in (await db.execute(_flag_q(Category.is_profit_share))).all()
        }
        # Spent for budget = total - marketing - profit_share.
        all_pids = set(spent_map) | set(marketing_map) | set(profit_share_map)
        spent_map = {
            pid: max(
                Decimal("0"),
                spent_map.get(pid, Decimal("0"))
                - marketing_map.get(pid, Decimal("0"))
                - profit_share_map.get(pid, Decimal("0")),
            )
            for pid in all_pids
        }
        ...
        po_rows = (
            await db.execute(
                select(PurchaseOrder.id, PurchaseOrder.project_id, PurchaseOrder.total).where(
        ...
        if po_ids:
            ...
            paid_per_po = {pid: Decimal(amt or 0) for pid, amt in (await db.execute(paid_q)).all()}
    ...
    summary = [
        {
            ...
            "value": f"Rp {_fmt_idr(total_budget)}",
            "sub": f"{len(projects)} proyek",
        },
        {
            "label": "Realisasi (VERIFIED)",
            "value": f"Rp {_fmt_idr(total_spent)}",
        ...
        {
            "label": "Sisa Real",
            "value": f"Rp {_fmt_idr(total_budget - overall_usage)}",
            "sub": f"{overall_pct:.1f}% terpakai",
        },
        {
            "label": "Status Risiko",
            "value": f"{n_over} overbudget",
    ...
    return await _output(
        ...
        footer_row=[
            ...
            "",
            "",
            _fmt_idr(total_budget),
            _fmt_idr(total_spent),
            _fmt_idr(total_committed),
            f"{overall_pct:.1f}%",
...
async def report_cash_advances(
    ...
    summary = [
        {
            ...
            "value": f"Rp {_fmt_idr(total_disbursed)}",
            "sub": f"{len(txs)} tx",
        },
        {
            "label": "Sudah Settled",
            "value": f"Rp {_fmt_idr(total_settled)}",
            "sub": f"{n_settled} / {len(txs)} tx",
        },
        {
            "label": "Outstanding",
            "value": f"Rp {_fmt_idr(total_outstanding)}",
            "sub": f"{n_outstanding} blm lapor",
        },
        {"label": "Avg Age Outstanding", "value": f"{avg_age:.0f} hari", "sub": "rata-rata umur"},
        {"label": "Aging Outstanding", "value": "lihat detail", "sub": aging_disp},
    ]
    proj_label = (
        proj_map.get(project_id).name if project_id and proj_map.get(project_id) else "Semua proyek"
    )
    period_label = (
        f"{_fmt_date(date_from) if date_from else 'awal'} s/d "
        f"{_fmt_date(date_to) if date_to else 'sekarang'}"
    )
    scope_line = f"Periode {period_label} · {proj_label} · {settlement_status or 'semua status'}"
    filters = {
    ...
    footer_row = [
        ...
        "",
        "",
        _fmt_idr(total_disbursed),
        "",
        _fmt_idr(total_settled),
        _fmt_idr(total_outstanding),
    ]
    company = await _resolve_company(db, project_id)
    return await _output(
        format,
        title="Laporan Dana Operasional",
        headers=headers,
        rows=rows,
        cols=cols,
        filters=filters,
        ...
        summary=summary,
        scope_line=scope_line,
        detail_label="Detail Dana Operasional",
...
async def report_audit(
    ...
    for log in logs:
        actions_count[log.action.value] = actions_count.get(log.action.value, 0) + 1
        rows.append(
            [
                _fmt_datetime(log.created_at),
                user_map.get(log.user_id).name if user_map.get(log.user_id) else "-",
                log.entity,
                str(log.entity_id),
                log.action.value,
                log.note or "-",
    ...
    summary = [
        {"label": "Total Entri", "value": str(len(rows)), "sub": "max 2.000 entri terbaru"},
        {"label": "User Unik", "value": str(len({log.user_id for log in logs})), "sub": ""},
        {"label": "Entity Unik", "value": str(len({log.entity for log in logs})), "sub": ""},
        {
