...
down_revision: str | Sequence[str] | None = "a5c7e9d2b3f4"
...
CONSTRAINTS = [
    ...
    ("transaction_items", "ck_transaction_items_amount_positive", "amount > 0"),
    (
        "cash_advance_settlements",
        "ck_cash_advance_settlements_returned_nonneg",
        "returned_to_kas >= 0",
    ),
    (
        "cash_advance_settlement_items",
        "ck_cash_advance_settlement_items_amount_positive",
        "amount > 0",
    ),
    ("cash_requests", "ck_cash_requests_total_nonneg", "total_amount >= 0"),
    ("cash_request_items", "ck_cash_request_items_amount_positive", "amount > 0"),
    ("invoices", "ck_invoices_subtotal_nonneg", "subtotal >= 0"),
    ("invoices", "ck_invoices_tax_nonneg", "tax >= 0"),
    ("invoices", "ck_invoices_total_nonneg", "total >= 0"),
    ("invoice_items", "ck_invoice_items_quantity_positive", "quantity > 0"),
    ("invoice_items", "ck_invoice_items_unit_price_nonneg", "unit_price >= 0"),
    ("purchase_orders", "ck_po_subtotal_nonneg", "subtotal >= 0"),
    ("purchase_orders", "ck_po_tax_nonneg", "tax >= 0"),
    ("purchase_orders", "ck_po_discount_nonneg", "discount >= 0"),
    ("purchase_orders", "ck_po_total_nonneg", "total >= 0"),
    ("po_items", "ck_po_items_quantity_positive", "quantity > 0"),
    ("po_items", "ck_po_items_unit_price_nonneg", "unit_price >= 0"),
]
