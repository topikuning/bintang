...
down_revision: str | Sequence[str] | None = "c4d2a9e1f7b8"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "total_amount", sa.Numeric(precision=18, scale=2), nullable=False, server_default="0"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["project_id"], ["projects.id"], name=op.f("fk_cash_requests_project_id_projects")
        ),
        sa.ForeignKeyConstraint(
            ["requester_id"], ["users.id"], name=op.f("fk_cash_requests_requester_id_users")
        ),
        sa.ForeignKeyConstraint(
            ["recipient_user_id"],
            ["users.id"],
            name=op.f("fk_cash_requests_recipient_user_id_users"),
        ),
        sa.ForeignKeyConstraint(
            ["approved_by_id"], ["users.id"], name=op.f("fk_cash_requests_approved_by_id_users")
        ),
        sa.ForeignKeyConstraint(
            ["rejected_by_id"], ["users.id"], name=op.f("fk_cash_requests_rejected_by_id_users")
        ),
        sa.ForeignKeyConstraint(
            ["disbursement_tx_id"],
            ["transactions.id"],
    ...
    op.create_index(
        op.f("ix_cash_requests_project_id"), "cash_requests", ["project_id"], unique=False
    )
    op.create_index(
        op.f("ix_cash_requests_requester_id"), "cash_requests", ["requester_id"], unique=False
    )
    op.create_index(op.f("ix_cash_requests_status"), "cash_requests", ["status"], unique=False)
    op.create_index(
        "ix_cash_requests_project_status", "cash_requests", ["project_id", "status"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["category_id"],
            ["categories.id"],
            name=op.f("fk_cash_request_items_category_id_categories"),
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_cash_request_items")),
    )
    op.create_index(
        op.f("ix_cash_request_items_request_id"), "cash_request_items", ["request_id"], unique=False
    )
